// Borrowed from https://github.com/henrygd/semaphore
...
const semaphoreMap = new Map() as Map<any, Semaphore>;
...
export const getSemaphore = (key: any = Symbol(), concurrency = 1): Semaphore => {
  ...
  const createPromise = (res: (value: void | PromiseLike<void>) => void) => {
    if (head && tail) {
      tail = tail.next = { res };
    } else {
      tail = head = { res };
